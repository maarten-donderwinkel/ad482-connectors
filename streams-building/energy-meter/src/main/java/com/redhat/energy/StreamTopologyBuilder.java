...
public class StreamTopologyBuilder {
    ...
    public Topology buildTopology() {
        ...
        ObjectMapperSerde<MWattsMeasurement> turbineMwattsSerde = new ObjectMapperSerde<>(MWattsMeasurement.class);

        // TODO: map the watts stream into a new mwatts stream
        wattsValuesStream.map((turbineId,watts) -> {
            Double mwatts = (double) watts / 1000000;
            MWattsMeasurement measurement = new MWattsMeasurement(turbineId,mwatts);
            System.out.println(
                    "MAP - Turbine: " + turbineId +
                            " | " + watts + " Watts -> " + mwatts + " MWatts"
            );

            return KeyValue.pair(turbineId, measurement);
        }).to(
                "turbine-generated-mwatts",
                Produced.with(Serdes.Integer(), turbineMwattsSerde)
        );

        // TODO: Create WindTurbineStats serde
        ObjectMapperSerde<WindTurbineStats> turbineStatsSerde = new ObjectMapperSerde<>(WindTurbineStats.class);
        ...
        wattsValuesStream
                ...
                .to(
                        "turbine-stats",
                        Produced.with(Serdes.Integer(), turbineStatsSerde)
                );
