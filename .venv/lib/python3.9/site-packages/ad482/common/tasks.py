...
def create_topics(
    ...
    try:
        for topic in topics:
            template_created = oc_client.apply_template(
                project,
                template_path, {
                    "TOPIC_NAME": topic,
            ...
            template_created.until_all(success_func=node_is_ready)
            successfully_created_topics.append(topic)
    except OcException as err:
        errors.append(err.get_messages)
    except Exception:
...
def topic_matches_specification(topic_name: str, partitions: int = None,
    ...
    if segment_bytes is not None and \
            (segment_bytes != topic[0].model.spec.config['segment.bytes']):
        raise TaskException(
            f"Configured segment bytes must be : {segment_bytes}"
        )

    if retention_ms is not None and \
            (retention_ms != topic[0].model.spec.config['retention.ms']):
        raise TaskException(
