...
import com.redhat.training.bank.event.*;
...
public class AmountWasWithdrawnPipeline extends StreamProcessor {
    ...
    void onStart(@Observes StartupEvent startupEvent) {
        ...
        mainStream.split()
                .branch((key, order) -> order.amount > 50 && order.amount <= 1000,Branched.withConsumer(this::processLowAmountEvents))
                .branch((key, order) -> order.amount > 1000 && order.amount < 3000,Branched.withConsumer(this::processModerateAmountEvents))
                .branch((key, order) -> order.amount > 3000,Branched.withConsumer(this::processHighAmountEvents));
